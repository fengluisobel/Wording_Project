(function () {
    ...
    diff_match_patch.prototype.diff_prettyHtml = function (a) {
        for (var b = [], ppp = [], c = /&/g, d = /</g, e = />/g, f = /\n/g, g = 0; g < a.length; g++) {
            var h = a[g][0],
                j = a[g][1],
                // j = j.replace(c, "&amp;").replace(d, "&lt;").replace(e, "&gt;").replace(f, "&para;<br>");

                // 看看这里换行符是不是有问题， 把段落符号去掉就好了
                j = j.replace(c, "&amp;").replace(d, "&lt;").replace(e, "&gt;").replace(f, "<br>");

            switch (h) {
                case 1:
                    // b[g] = '<ins style="background:rgb(196,215,155);">' + j + "</ins>";
                    b[g] = '<ins style="color:rgb(0,32,96);">' + j + "</ins>";
                    
                    break;
                case -1:
                    // b[g] = '<del style="background:rgb(252,213,180);">' + j + "</del>";
                    // 看看要不要把这个变成悬浮的
                    // 试一下
                    // b[g] = '<del style="color:rgb(252,213,180);background:rgb(252,213,180);">' + j + "</del>";


                    b[g] = '<del style="color:rgb(226,107,10);">' + j + "</del>";


                    // ppp[g] = '<a class="tooltip" id="tooltip1" style="background:rgb(255,255,0);">' + j + "</del>";
